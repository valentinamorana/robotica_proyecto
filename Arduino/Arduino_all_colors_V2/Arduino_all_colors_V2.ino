...
CRGB    baseColor = CRGB::Blue;   // default azul
...
// >>> Modo audio-rítmico puro (ignora color base en SOUND ON)
bool audioPure = true;
...
void applyCmd(const String& cmdRaw){
  ...
  if(cmd=="MODE?"){
    ...
    Serial.print("AudioPure: "); Serial.println(audioPure?"true":"false");
    return;
  }
  ...
  if(cmd.startsWith("THRESH")){
    int sep = cmd.lastIndexOf(' ');
    int val = (sep>0)? cmd.substring(sep+1).toInt() : soundThreshold;
    val = constrain(val, 0, 1022); // evita divisor 0 en el map
    soundThreshold = val;
    Serial.print("Umbral sonido = "); Serial.println(soundThreshold);
    return;
  }

  // Estilo de sonido (se mantiene por si querés usarlo con audioPure=false)
  if(cmd == "SOUND STYLE BRIGHT"){ 
    soundStyle = SOUND_STYLE_BRIGHT; 
    Serial.println("🔊 Sound style: BRIGHT (solo brillo)"); 
    return; 
  }
  if(cmd == "SOUND STYLE HUE"){ 
    soundStyle = SOUND_STYLE_HUE; 
    Serial.println("🔊 Sound style: HUE (oscila tono)"); 
    return; 
  }

  // Dirección (coincidencia exacta para evitar RIGHT en BRIGHT)
  if(cmd == "ALL"){    direction=0; Serial.println("Dirección: TODAS");   return; }
  if(cmd == "LEFT"){   direction=1; Serial.println("Dirección: IZQUIERDA");return; }
  if(cmd == "RIGHT"){  direction=2; Serial.println("Dirección: DERECHA"); return; }
  if(cmd == "CENTER"){ direction=3; Serial.println("Dirección: CENTRO");  return; }

  // Sound ON / OFF
  if(cmd.indexOf("SOUND ON")>=0 || cmd.indexOf("MIC ON")>=0){
    modeSel = MODE_SOUND; 
    rainbowMode = false;
    audioPure = true;   // << CLAVE: ignora color manual, RGB audio-rítmico
    Serial.println("🔊 Modo sonido: audio-rítmico RGB (sin color base)");
    return;
  }
  if(cmd.indexOf("SOUND OFF")>=0 || cmd.indexOf("MIC OFF")>=0){
    modeSel = MODE_MANUAL;
    audioPure = false;  // vuelve al manual con el color elegido
  ...
  if(cmd=="HELP"){
    ...
    Serial.println("THRESH:0..1023 (internamente clamp a 1022)");
...
void loop(){
  ...
  // Modo sonido: AUDIO-RÍTMICO RGB PURO cuando audioPure==true
  if(modeSel==MODE_SOUND && !rainbowMode){
    int thr = constrain(soundThreshold, 0, 1022);
    int soundLevel = analogRead(MIC_PIN);
    int levelAbove = max(0, soundLevel - thr);

    // Brillo según nivel de sonido (no pisa FastLED.setBrightness global)
    uint8_t val = (uint8_t)constrain(map(levelAbove, 0, 1023 - thr, 8, 255), 0, 255);

    static uint8_t rollingHue = 0;    // rueda continua de color
    rollingHue++;
    uint8_t delta = (uint8_t)constrain(map(levelAbove, 0, 1023 - thr, 0, 96), 0, 96);

    CHSV hsv;
    if (audioPure) {
      // >>> RGB audio-rítmico: ignora color manual
      uint8_t h = rollingHue + delta; // color rota y responde al volumen
      hsv = CHSV(h, 255, val);
    } else {
      // (Disponible si alguna vez querés respetar el color base)
